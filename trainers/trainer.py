...
def trainer(config, model, train_loader, val_loader, test_loader, class_wts, device, logger, save_model_name, ckpt_dir):
    ...
    for epoch in range(config['start_epoch'], config['total_epochs']):
        ...
        #train model
        train_loss, train_accuracy, train_microf1, train_macrof1, train_f1 = train_Bert(model, train_loader, device, cross_entropy, optimizer)
        ...
        #evaluate model
        valid_loss, valid_accuracy, valid_microf1, valid_macrof1, valid_f1 = evaluate_Bert(model, val_loader, device, cross_entropy)
        ...
        test_loss, test_accuracy, test_microf1, test_macrof1, test_f1 = test_Bert(model, test_loader, device, cross_entropy)
    ...
    # loss_curve(train_losses, valid_losses, train_accuracies, val_accuracies)
...
def train_Bert(model, train_loader, device, cross_entropy, optimizer):
    ...
    return avg_loss, avg_accuracy, micro_f1, macro_f1, average_f1
...
# def test_Bert_2(model, test_loader, device, cross_entropy, ckpt_dir, save_model_name):
  
#     print("\nTesting...")
#     t0 = time.time()

#     model_path = os.path.join(ckpt_dir, 'best_test_'+ save_model_name)
#     trainable_params = torch.load(model_path)
